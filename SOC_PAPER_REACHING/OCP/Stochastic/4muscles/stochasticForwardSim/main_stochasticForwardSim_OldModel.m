...
for j = 1:length(listing)
    j/length(listing)
    if contains(listing(j).name,'_OldModel.mat')
        name = listing(j).name;
        load(name);
        if isfield(result,'X_stoch_unperturbed')
            functions = generateFunctions_OCP_4muscles_FF_FB_OldModel(result.auxdata);
...
%                 [X_stoch_local,EE_stoch_local] = stochasticForwardSim_4muscles_OldModel(result.X(:,1),result.e_ff',result.EE_ref,result.K,result.auxdata,functions,perturbation);
...
%             end
            ...
            parfor i = 1:nrStochSims
                [X_stoch_local,EE_stoch_local] = stochasticForwardSim_4muscles_OldModel(result.X(:,1),result.e_ff',result.EE_ref,result.K,result.auxdata,functions,perturbation);
                EE_stoch_perturbed_2(:,:,i) = EE_stoch_local;
                X_stoch_perturbed_2(:,:,i) = X_stoch_local;
            end
...
%             result.X_stoch_perturbed = X_stoch_perturbed;
            result.EE_stoch_perturbed_2 = EE_stoch_perturbed_2;
            result.X_stoch_perturbed_2 = X_stoch_perturbed_2;
            save(name,'result')
        end
    end
end
