...
for i = 1:length(wM_std_VEC)
    for j = 1:length(wPq_std_VEC)
        for s = 1:length(wPqdot_std_VEC)
            ...
            titles = {'CIRCLE unperturbed','BAR unperturbed','OBSTACLE unperturbed'};
            ...
            if isfield(result,'X_stoch_unperturbed')
                for k=2:9:size(result.EE_stoch_unperturbed,3)
...
%                     error_ellipse(C,mu,0.95,1,'style','-.');
            ...
            if isfield(result,'X_stoch_perturbed')
                for k=1:1:size(result.EE_stoch_unperturbed,3)
                    scatter(result.EE_stoch_unperturbed(1,end,k)-result.EE_ref(1,end),result.EE_stoch_unperturbed(2,end,k)-result.EE_ref(2,end),'x','MarkerEdgeColor',cs(1,:)); hold on;
                end
...
%                     error_ellipse(C,mu,0.95,1,'style','-.');
            ...
            if isfield(result,'X_stoch_perturbed')
                for k=1:1:size(result.EE_stoch_unperturbed,3)
                    scatter(result.EE_stoch_unperturbed(1,end,k)-result.EE_ref(1,end),result.EE_stoch_unperturbed(2,end,k)-result.EE_ref(2,end),'x','MarkerEdgeColor',cs(2,:)); hold on;
                end
...
%                     error_ellipse(C,mu,0.95,1,'style','-.');
            ...
            if isfield(result,'X_stoch_perturbed')
                for k=1:1:size(result.EE_stoch_unperturbed,3)
                    scatter(result.EE_stoch_unperturbed(1,end,k)-result.EE_ref(1,end),result.EE_stoch_unperturbed(2,end,k)-result.EE_ref(2,end),'x','MarkerEdgeColor',cs(3,:)); hold on;
                end
