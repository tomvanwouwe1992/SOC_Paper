...
for i = 1:length(wM_std_VEC)
    for j = 1:length(wPq_std_VEC)
        for k = 1:length(wPqdot_std_VEC)
            ...
            %             guessName = ['result_time_0.8_' target '_forceField_' num2str(forceField) '_' num2str(wM_std) '_' num2str(wPq_std) '_' num2str(wPqdot_std) '.mat'];
            guessName = [];
            ...
            target = 'OBSTACLE';
            saveName = ['result_time_0.8_' target '_forceField_' num2str(forceField) '_' num2str(wM_std) '_' num2str(wPq_std) '_' num2str(wPqdot_std) '_OldModel_.mat'];
            %             guessName = ['result_time_0.8_CIRCLE_forceField_' num2str(forceField) '_' num2str(wM_std) '_' num2str(wPq_std) '_' num2str(wPqdot_std) '_OldModel.mat'];
            guessName = [];
            if ~contains( [listing(:).name] ,saveName )
                [result,succes] = OCP_4muscles_FF_FB_final_OldModel(target,forceField,wM_std,wPq_std,wPqdot_std,guessName);
                if succes == true
                    parsave(saveName,result);
                end
            end
