function [result,succes] = OCP_4muscles_FF_FB_final_OldModel_DCversion(target,forceField,wM_std,wPq_std,wPqdot_std,guessName)
%% SECTION TITLE
% DESCRIPTIVE TEXT
...
auxdata = initializeModelParameters_4muscles_OldModel();
auxdata.FV = forceField;
...
indices_P = [1 2 3 4 5 6 7 8;  0 9 10 11 12 13 14 15 ; 0 0 16 17 18 19 20 21 ; 0 0 0 22 23 24 25 26 ; 0 0 0 0 27 28 29 30; 0 0 0 0 0 31 32 33; 0 0 0 0 0 0 34 35; 0 0 0 0 0 0 0 36]';
vecIndices_P = [1 2 3 4 5 6 7 8 10 11 12 13 14 15 16 19 20 21 22 23 24 28 29 30 31 32  37 38 39 40 46 47 48 55 56 64]';


%%%% Define CasADi functions - for reasons of efficiency and to compute sensitivities (jacobians) of functions
functions = generateFunctions_OCP_4muscles_FF_FB_OldModel(auxdata);
...
for i = 1:N
    ...
    Lmat_i = [L(indices_P(:,1),i) [zeros(1,1);L(indices_P(2:end,2),i)] [zeros(2,1);L(indices_P(3:end,3),i)] [zeros(3,1);L(indices_P(4:end,4),i)] [zeros(4,1);L(indices_P(5:end,5),i)] [zeros(5,1);L(indices_P(6:end,6),i)] [zeros(6,1);L(indices_P(7:end,7),i)] [zeros(7,1);L(indices_P(8,8),i)]];
    Lmat_next = [L(indices_P(:,1),i+1) [zeros(1,1);L(indices_P(2:end,2),i+1)] [zeros(2,1);L(indices_P(3:end,3),i+1)] [zeros(3,1);L(indices_P(4:end,4),i+1)] [zeros(4,1);L(indices_P(5:end,5),i+1)] [zeros(5,1);L(indices_P(6:end,6),i+1)] [zeros(6,1);L(indices_P(7:end,7),i+1)] [zeros(7,1);L(indices_P(8,8),i+1)]];
...
J_fb = J_fb + (functions.f_expectedEffort_fb(X_i_plus,Pmat_next,K_i_plus,EE_ref_i_plus,wPq,wPqdot) + trace(Pmat_next(1:4,1:4)))/2;% + trace(Pmat_i(1:6,1:6));
...
% target shape)
P_q_final = Pmat_next(5:6,5:6);
P_EEPos_final = functions.f_P_EEPos(X(5:6,end),P_q_final);
P_q_qdot_final = Pmat_next(5:8,5:8);
...
try
    ...
    % using DC
    for i = 1:N+1
        Lmat_i = [L_sol(indices_P(:,1),i) [zeros(1,1);L_sol(indices_P(2:end,2),i)] [zeros(2,1);L_sol(indices_P(3:end,3),i)] [zeros(3,1);L_sol(indices_P(4:end,4),i)] [zeros(4,1);L_sol(indices_P(5:end,5),i)] [zeros(5,1);L_sol(indices_P(6:end,6),i)] [zeros(6,1);L_sol(indices_P(7:end,7),i)] [zeros(7,1);L_sol(indices_P(8,8),i)]];
        Pmat_i = Lmat_i*Lmat_i';
