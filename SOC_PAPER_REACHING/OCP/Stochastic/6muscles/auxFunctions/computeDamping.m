function Damping = computeDamping(resultName)
import casadi.*
...
for t = 1:5:66
    ...
    for theta = 0:stepAngle:2*pi-stepAngle
        theta_i = theta_i + 1;
        pert_v = 0.05*[cos(theta);sin(theta)];
        J = full(f_JAC_EEPos(result.q(t,:)'));
        delta_qdot = J^(-1)*pert_v;
        ...
        for k = 1:15
            if t+k-1 > 81
                ...
                e_fb(:,k+1) = reshape(result.K(:,t+k-1),6,4)*([zeros(2,1); pert_v]);
        ...
        for k = 1:15
            ...
            rf = rootfinder('rf','newton',struct('x',sol_T_EXT_MX,'g',f_forwardMusculoskeletalDynamics([a_sim(:,k);result.q(index,:)';result.qdot(index,:)'+delta_qdot],result.e_ff(t,:)',reshape(result.K(:,index),6,4),sol_T_EXT_MX,result.EE_ref(:,index),zeros(2,1),zeros(2,1),zeros(2,1))),struct('abstol',1e-16));
            solution = rf(zeros(2,1),[]);
            
            J = full(f_JAC_EEPos(result.q(index,:)')); % xdot = J*qdot
            ...
            F_restore_norm(:,k) = pert_v'*F_restore(:,k)/(norm(pert_v));
            Damping_trial(:,k) = F_restore_norm(:,k)/norm(pert_v);
            [Fa_perturbed(:,k),Fp_perturbed(:,k),~,~,~,~,~,~,~] = getMuscleForce(result.q(index,:)',result.qdot(index,:)'+delta_qdot,result.auxdata);
            
        end
        Damping(t_i,theta_i) = Damping_trial(15);
    end
end
end
