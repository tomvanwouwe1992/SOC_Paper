function [result,succes] = OCP_6muscles_FF_FB_final_DCversion(target,forceField,wM_std,wPq_std,wPqdot_std,guessName)
%% SECTION TITLE
% DESCRIPTIVE TEXT
...
if isempty(guessName)
    ...
    K = opti.variable(4*4,N+1); opti.set_initial(K, 0.001);
    ...
    if abs(EE_ref_guess(1,end)) > 0.1 % control of initial guess is not very stable/precise, better to violate the dynamics and provide trajectories that satisfy the constraints
        ...
        L = opti.variable(nStates*(nStates+1)/2,N+1);
        % Bit more involved for the covariance trajectory
        Pmat_init = [1e-6;1e-6;1e-6;1e-6;1e-6;1e-6;1e-4;1e-4;1e-7;1e-7].*eye(10);
        Pvec_init = Pmat_init(vecIndices_P);
        L_init = sqrt(Pvec_init);
        opti.subject_to(L(:,1) == L_init);
        for i = 1:N+1
            Pmat_guess_i = Pmat_guess(:,:,i);
            Lmat_guess_i = chol(Pmat_guess_i)';
            opti.set_initial(L(:,i),Lmat_guess_i(vecIndices_P));
        end
    else
        X_init = opti.variable(nStates,1); opti.set_initial(X_init, X_init_guess);
        X = opti.variable(nStates,N+1); opti.set_initial(X, X_guess);
...
for i = 1:N
    ...
    Lmat_i = [L(indices_P(:,1),i) [zeros(1,1);L(indices_P(2:end,2),i)] [zeros(2,1);L(indices_P(3:end,3),i)] [zeros(3,1);L(indices_P(4:end,4),i)] [zeros(4,1);L(indices_P(5:end,5),i)] [zeros(5,1);L(indices_P(6:end,6),i)] [zeros(6,1);L(indices_P(7:end,7),i)] [zeros(7,1);L(indices_P(8:end,8),i)] [zeros(8,1);L(indices_P(9:end,9),i)] [zeros(9,1);L(indices_P(10,10),i)]];
    Lmat_next = [L(indices_P(:,1),i+1) [zeros(1,1);L(indices_P(2:end,2),i+1)] [zeros(2,1);L(indices_P(3:end,3),i+1)] [zeros(3,1);L(indices_P(4:end,4),i+1)] [zeros(4,1);L(indices_P(5:end,5),i+1)] [zeros(5,1);L(indices_P(6:end,6),i+1)] [zeros(6,1);L(indices_P(7:end,7),i+1)] [zeros(7,1);L(indices_P(8:end,8),i+1)] [zeros(8,1);L(indices_P(9:end,9),i+1)] [zeros(9,1);L(indices_P(10,10),i+1)]];

    Pmat_i = Lmat_i*Lmat_i';
    Pmat_next = Lmat_next*Lmat_next';
    Pmat_next_integrated = M_i*(DG_DX_i*Pmat_i*DG_DX_i' + DG_DW_i*sigma_w*DG_DW_i')*M_i';

    Pvec_next_integrated = Pmat_next_integrated(vecIndices_P);
    Pvec_next = Pmat_next(vecIndices_P);

    opti.subject_to(Pvec_next_integrated == Pvec_next);
    ...
    J_fb = J_fb + (functions.f_expectedEffort_fb(X_i,Pmat_i,K_i,EE_ref_i,wPq,wPqdot) + trace(Pmat_i(1:6,1:6)))/2;% + trace(Pmat_i(1:6,1:6)); %expectedEffort_fb(i);
...
opti.subject_to(X_init(7:10) - [initial_pos;0;0] == 0); % Initial position and velocity
dX_init = functions.f_forwardMusculoskeletalDynamics(X_init,e_ff(:,1),0,0,0*wM,0*wPq,0*wPqdot); % Initial state derivative
opti.subject_to(dX_init(9:10) == 0); % Initial acceleration equals zero (activations need to fullfill requirement)

% Reaching motion must end in the final reach position with zero angular joint velocity
opti.subject_to(functions.f_EEPos(X(7:8,end)) - EE_final == 0);
opti.subject_to(X(9:10,end) == [0;0]);
...
opti.subject_to(dX_end(9:10) == 0); % Initial acceleration equals zero (activations need to fullfill requirement)
...
% target shape)
P_q_final = Pmat_next(7:8,7:8);
P_EEPos_final = functions.f_P_EEPos(X(7:8,end),P_q_final);
P_q_qdot_final = Pmat_next(7:10,7:10);
...
optionssol.ipopt.max_iter = 100000;
...
%     opti.callback(@(i) opti.debug.show_infeasibilities(1e-1))
    ...
    % Compare forward sim of the covariance with the found optimal
    % controller to the trajectory found within the optimization solved
    % using DC
    for i = 1:N+1

        Lmat_i = [L_sol(indices_P(:,1),i) [zeros(1,1);L_sol(indices_P(2:end,2),i)] [zeros(2,1);L_sol(indices_P(3:end,3),i)] [zeros(3,1);L_sol(indices_P(4:end,4),i)] [zeros(4,1);L_sol(indices_P(5:end,5),i)] [zeros(5,1);L_sol(indices_P(6:end,6),i)] [zeros(6,1);L_sol(indices_P(7:end,7),i)] [zeros(7,1);L_sol(indices_P(8:end,8),i)] [zeros(8,1);L_sol(indices_P(9:end,9),i)] [zeros(9,1);L_sol(indices_P(10,10),i)]];
        Pmat_i = Lmat_i*Lmat_i';
        diff = norm(Pmat_i - Pmat_sol(:,:,i));
        if diff > 1e-4
            disp(['Too large difference between forward and DC simulation of the covariance from mesh interval ' num2str(i)]);
        end
    end
